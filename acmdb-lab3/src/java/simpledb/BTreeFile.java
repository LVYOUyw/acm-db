...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		// some code goes here
		if (pid.pgcateg() == 2) { //the leaf page
			return (BTreeLeafPage)this.getPage(tid, dirtypages, pid, perm);
		}					
		else if (pid.pgcateg() == 1) {
			BTreeInternalPage BIP = (BTreeInternalPage)this.getPage(tid, dirtypages, pid, Permissions.READ_ONLY);
			BTreeInternalPageIterator it = (BTreeInternalPageIterator)BIP.iterator();
			BTreeEntry BE = null;
			if (f == null) {
				BE = it.next();
				return findLeafPage(tid, dirtypages, BE.getLeftChild(), Permissions.READ_ONLY, f);
			}
		//	IndexPredicate cmp = new IndexPredicate(Predicate.Op.LESS_THAN_OR_EQ, f);
			while (it.hasNext()) {
				BE = it.next();
				if (f.compare(Predicate.Op.LESS_THAN_OR_EQ, BE.getKey()))
					return findLeafPage(tid, dirtypages, BE.getLeftChild(), Permissions.READ_ONLY, f);
			}
			if (BE != null) 
				return findLeafPage(tid, dirtypages, BE.getRightChild(), Permissions.READ_ONLY, f); 
		}
        return null;
	}
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		BTreeLeafPageReverseIterator it = (BTreeLeafPageReverseIterator)page.reverseIterator(); 
		BTreeLeafPage rightpage = (BTreeLeafPage)getEmptyPage(tid, dirtypages, BTreePageId.LEAF);
		int size = page.getNumTuples();
		...
		while (splitsize > 0) {
			...
			rightpage.insertTuple(tmp);
			splitsize--;
		}
		//BTreeLeafPageReverseIterator newit = (BTreeLeafPageReverseIterator)page.reverseIterator();
		lastT = it.next();
		...
		if (page.getRightSiblingId() != null) {
			...
			originalrightpage.setLeftSiblingId(rightpage.getId());
			rightpage.setRightSiblingId(originalrightpage.getId());
		}
		page.setRightSiblingId(rightpage.getId());
		BTreeEntry BE = new BTreeEntry(lastT.getField(keyField), page.getId(), rightpage.getId());
		BTreeInternalPage BI = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), BE.getKey());
		BI.insertEntry(BE);
		dirtypages.put(BI.getId(), BI);
		page.setParentId(BI.getId());
		rightpage.setParentId(BI.getId());
		if (field.compare(Predicate.Op.GREATER_THAN, BE.getKey()))
			return rightpage;
        return page;
	}
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
			BTreeInternalPage page, Field field) 
					throws DbException, IOException, TransactionAbortedException {
		// some code goes here
		BTreeInternalPageReverseIterator it = (BTreeInternalPageReverseIterator)page.reverseIterator(); 
		BTreeInternalPage rightpage = (BTreeInternalPage)getEmptyPage(tid, dirtypages, BTreePageId.INTERNAL);
		int size = page.getNumEntries();
		int splitsize = size / 2;
		BTreeEntry tmp = null;
		while (splitsize > 0) {
			tmp = it.next();
			page.deleteKeyAndRightChild(tmp);
			rightpage.insertEntry(tmp);
			splitsize--;
		}
		dirtypages.put(page.getId(), page);
		//BTreeInternalPageReverseIterator newit = (BTreeInternalPageReverseIterator)page.reverseIterator();
		BTreeEntry lastT = it.next();
		BTreeEntry newnode = new BTreeEntry(lastT.getKey(), page.getId(), rightpage.getId());
		page.deleteKeyAndRightChild(lastT);
		BTreeInternalPage BI = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), newnode.getKey());
		BI.insertEntry(newnode);
		dirtypages.put(BI.getId(), BI);
		page.setParentId(BI.getId());
		rightpage.setParentId(BI.getId());
		...
		if (field.compare(Predicate.Op.GREATER_THAN, newnode.getKey()))
			return rightpage;
        return page;
	}
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		if (isRightSibling) {
			...
			BTreeLeafPageReverseIterator rit = (BTreeLeafPageReverseIterator)page.reverseIterator();
			tmp = rit.next();
			entry.setKey(tmp.getField(keyField));
		}
		else {
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// some code goes here
		int numsteal = (page.getNumEntries() + leftSibling.getNumEntries()) / 2 - page.getNumEntries() - 1;
		BTreeInternalPageIterator it = (BTreeInternalPageIterator)page.iterator();
		BTreePageId lpid = it.next().getLeftChild();
		BTreeInternalPageReverseIterator rit = (BTreeInternalPageReverseIterator)leftSibling.reverseIterator();
		BTreeEntry BE = rit.next();
		BTreePageId rpid = BE.getRightChild();
		BTreeEntry TmpE = new BTreeEntry(parentEntry.getKey(), rpid, lpid);
		page.insertEntry(TmpE);
		...
		//BE = rit.next();
		BTreeEntry TopE = new BTreeEntry(BE.getKey(), parentEntry.getLeftChild(), parentEntry.getRightChild());
		parent.deleteKeyAndRightChild(parentEntry);	
		leftSibling.deleteKeyAndRightChild(BE);
		parent.insertEntry(TopE);
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		int numsteal = (page.getNumEntries() + rightSibling.getNumEntries()) / 2 - page.getNumEntries() - 1;
		BTreeInternalPageReverseIterator it = (BTreeInternalPageReverseIterator)page.reverseIterator();
		BTreePageId lpid = it.next().getRightChild();
		BTreeInternalPageIterator rit = (BTreeInternalPageIterator)rightSibling.iterator();
		BTreeEntry BE = rit.next();
		BTreePageId rpid = BE.getLeftChild();
		BTreeEntry TmpE = new BTreeEntry(parentEntry.getKey(), lpid, rpid);
		page.insertEntry(TmpE);
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		if (rightPage.getRightSiblingId() != null) {
			BTreeLeafPage updater = (BTreeLeafPage) getPage(tid, dirtypages, rightPage.getRightSiblingId(), Permissions.READ_WRITE);
			updater.setLeftSiblingId(leftPage.getId());
			leftPage.setRightSiblingId(updater.getId());
			dirtypages.put(updater.getId(), updater);
			dirtypages.put(leftPage.getId(), leftPage);
		}
		else leftPage.setRightSiblingId(null); 

		BTreeLeafPageIterator rit = (BTreeLeafPageIterator)rightPage.iterator();
		Tuple Tp = null;
		while (rit.hasNext()) {
			Tp = rit.next();
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// Delete the entry in the parent corresponding to the two pages that are merging -
		// deleteParentEntry() will be useful here
		BTreeInternalPageReverseIterator it = (BTreeInternalPageReverseIterator)leftPage.reverseIterator();
		BTreePageId lpid = it.next().getRightChild();
		BTreeInternalPageIterator rit = (BTreeInternalPageIterator)rightPage.iterator();
		BTreeEntry BE = rit.next();
		BTreePageId rpid = BE.getLeftChild();
		BTreeEntry TmpE = new BTreeEntry(parentEntry.getKey(), lpid, rpid);
		leftPage.insertEntry(TmpE);
		rightPage.deleteKeyAndRightChild(BE);
