    ...
    public Vector<LogicalJoinNode> orderJoins(
        ...
        for (int i = 1; i <= this.joins.size(); i++)
        {
            for (Set<LogicalJoinNode> s : this.enumerateSubsets(this.joins, i))
            {
                ...
                int cur = 0;
                for (LogicalJoinNode tmp : s)
                {
                    CostCard cc = this.computeCostAndCardOfSubplan(stats, filterSelectivities, tmp, s, bestCost, this.planc);
                    if (cc == null)
                        continue;
                    if (cc.cost < bestCost)
                    {
                ...
                this.planc.addPlan(s, bestCost, cur, bestPlan);
            }
        }
        HashSet<LogicalJoinNode> j = new HashSet<LogicalJoinNode>(this.joins);
        return this.planc.getOrder(j);
    }
